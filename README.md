...
<video src="https://github.com/ymhHAHA/KnowX/raw/main/showcase/20260510\\\_171518.mp4" controls width="90%">

&#x20; 您的浏览器不支持内嵌视频播放，请<a href="https://github.com/ymhHAHA/KnowX/raw/main/showcase/20260510\\\_171518.mp4">下载演示视频</a>观看。
...
!\[KnowX Architecture](showcase/KnowX\_Architecture.png)
...
1. 用户登录后创建知识库，上传文档（可附加 `custom\\\_metadata`）。
2. 后端将解析任务投递到 `parse\\\_index\\\_queue`，Celery Worker 异步执行。
...
|`vector\\\_only`|关闭 KG，仅向量 + rerank|图谱质量不稳定、纯语义检索|
...
* **租户隔离**：每个知识库绑定 `owner\\\_id`，跨租户访问统一返回 404
* **鉴权图片代理**：文档图片不经公开静态目录暴露，通过 `/api/v1/documents/image-file/{workspace\\\_id}/{file}` 并校验 Bearer Token
...
这些元数据写入 ChromaDB 的 chunk 索引，检索 API 支持 `metadata\\\_filter`，可先按业务维度过滤再做向量检索，**减少无关 chunk 进入 LLM 上下文**。
...
* `search\\\_documents` 工具调用，将回答约束在检索结果上
* 工作区级自定义 system prompt
* 多轮对话历史
* 强制检索模式 `force\\\_search`
* 支持时展示模型 thinking 内容
* 聊天历史持久化与消息评分
...
|Tool|说明|
|-|-|
|`get\\\_workspace\\\_list`|列出所有知识库|
|`get\\\_document\\\_by\\\_id`|获取指定文档信息|
|`query`|对索引文档进行语义检索|
...
**轨道 1 — 32 题手工精标**（通过阈值 overall\_score ≥ 0.70，Judge：DeepSeek-Chat）：
...
|指标|均值|说明|
|-|-|-|
|`context\\\_recall`|0.697|检索召回尚可，长尾在表格与 metadata|
|`factual\\\_correctness` (F1)|0.791|含 RAGAS judge 假阴性，估计真实值约 0.6–0.7|
|`faithfulness`|0.688|偶发超出检索内容的发挥|
|`table\\\_extraction` recall|0.63|表格/数值题最弱，为当前首要改进项|

<details>
<summary>已识别的主要短板与改进方向</summary>

1. **检索精度 / metadata**：封面、出版日期类 query 召回不稳；`context\\\_relevancy` 偏低
2. **长综合答案引用**：跨文档综合题内容正确但部分缺少 inline `\\\[xxxx]` 引用
3. **表格与数值**：`table\\\_extraction` 类型 recall/FC 最低；表格易被 chunk 打散
4. **Multi-hop**：`multi\\\_hop\\\_reasoning` recall 约 0.60，低于 single-hop
